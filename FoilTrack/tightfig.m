...
function [pos, ti] = colorbarpos (hcbar)
    ...
    %SAH edit: to make process manuel colour bar positions.
    if ~isempty (strfind (hcbar.Location, 'manual'))
        if strcmp (hcbar.AxisLocation, 'out')
            ...
            ylab = get(hcbar,'ylabel');
            yalbsize = ylab.FontSize;
            ...
            ylablespace_cm = (0.3528/10) * yalbsize * 2.5; %2.5 is an artitrary number I chose to make it work
            ti(3) = ti(3) + ticklablespace_cm + ylablespace_cm;
        end
        
    elseif ~isempty (strfind (hcbar.Location, 'outside'))
        if strcmp (hcbar.AxisLocation, 'out')
            ...
            switch hcbar.Location
                ...
                case 'eastoutside'
                    ...
                    % know the width of every character in the label, the
                    % fsize refers to the height of the font
                    ticklablespace_cm = (0.3528/10) * fsize * maxlabellen * 2.62;
