...
filevariables = {'length_type', 'get_rid', 'symm_type', 'bg_type', 'excessive', 'err_calc', 'scaling', 'search'};
...
[detrended, BackGround] = Phases_detrend(vars.bg_type, length_change, image_time, arg_pass{:});
...
%     case 'polynomial'
%         for x = 1:number_fits_todo
%             poly_average_coef = polyfit(image_time,length_change(:,x),5);
...
if fixed == 0
    ...
    [~,y_min_pos] = find(over_all == min_min_range);
...
amplitudes = [sqrt(mean(detrended.^2,1)); zeros(1,number_fits_todo)];

%tidy the amplitudes
[phases,amplitudes(1,:)] = Sinusoid_tidy(phases, amplitudes(1,:));
...
%     fit_all_data = fminsearch(@SineSQ_SSD,inputs,minimise_options,phase_rad,detrended(:,x),vars.excessive);
    fit_all_data = fminsearch(@SineSSD,inputs,minimise_options,image_time,detrended(:,x),period,vars.excessive);
...
%     [SSD(x) ~] = SineSQ_SSD(fit_all_data,phase_rad,detrended(:,x),vars.excessive); %calculates SSD for fit
    [SSD(x) ~] = SineSSD(fit_all_data,image_time,detrended(:,x),period,vars.excessive); %calculates SSD for fit
...
% make sure that all the phase'plot' values reported are less than +/- 2pi.
% phases = mod(phases,2*pi);
[phases, amplitudes(1,x)] = Sinusoid_tidy(phases, amplitudes(1,x));
...
y = 1;
phase_not_bad = []; %set this variable so that is alredy defined.
for x = 1 : number_fits_todo
    if fixed == 1
        useable(x) = 0;
    elseif SSD(x) <= max_allowed_SSD
        useable(x) = 1;
...
if fixed == 0 || number_pairs_not_bad ~= 0
    if ratio_bar >= cutoff
...
%         if ismember(x,fit_plot) == 1
            figure(gcf);
            clf;
            subplot(2,1,1); %cla image_time
                % plot(phase_time,detrended(:,x),'r.', phase_time,allcalc,'b.');
                plot(image_time,detrended(:,x),':b.', image_time,allcalc,'r-');
            ...
            subplot(2,1,2); %cla
                ...
                plot(image_time, miss_fit,':g.',[0 image_time(end)],[0 0],'k-');
                xlabel('Phase (radians)');
                ylabel('delta length');
                title(strcat('pair ',num2str(x), ' - detrended data')); 
                %xlim([0 2*pi]);
                hold on;
...
write_file_name = [out_file_name,'_FITS.mat'];
save(write_file_name, 'BackGround', 'length_change', 'output_values', 'image_time', 'output_vars');
...
%% Versioning
% v4.3.alpha.2 - 11 June 2018
%   - Fixed phase guess - which was wrong because of change from cos to sin
%   when calling Phases_SneFunc
% v4.3.alpha - 8 June 2018
%   - Removed detrending to separate script.
% v4.2.alpha - 19 October 2017
%   - Added lines so that the fitted backgrounds are saved to *.mat file.
% v4.1.alpha - 10 July 2017
%   - Fixed code so that 'fixed' as an option now keeps the period fixed.
% v4.alpha --  July 2017
%   - Replaced the sine functions in the code with calls to
%   Phases_SineFunc. This is so that the code should output the same
%   answers as PhasesSSD, for the same data set. 
% v3.2 - 26 June 2017
