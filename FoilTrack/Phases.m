...
%
%   syntax: Phases(displaements_file_name.txt, options)
...
%
%   See also: ImageAnalysis, PhaseCheck, PhasesSSD

%   $ version: 3.2 $ 26 June 2017 $
...
function Phases(file_name, varargin)
tic
...
for x = 1 : length(useable)
    if useable(x) == 0
        ...
        results = fminsearch(MODELb,inputs,minimise_options,phase_time,detrended(:,x),vars.excessive);
        phases(x) = results(1);
...
for x = 1:number_fits_todo
    ...
    [SSD(x) nos(x)] = MODELb(coefficients, phase_time, detrended(:,x), vars.excessive);
    ...
    if strcmpi(vars.err_calc,'yes') == 1
        ...
        % fiting function.
        out = errors(phase_time,detrended(:,x),coefficients);
        phase_err(x) = out(1);
        amp_err(x) = out(2);
    ...
    if plots_on == 1
        ...
        if ismember(x,fit_plot) == 1
            ...
            clf;
            subplot(2,1,1); %cla
                plot(image_time,detrended(:,x),'r.', image_time,allcalc,'b-');
                xlabel('Time (s)');
                ylabel('delta length');
                title(strcat('pair',num2str(x), ' - global fit results'));
                %xlim([0 2*pi]);
            subplot(2,1,2); %cla
                plot(image_time, detrended(:,x)-allcalc,'g-',[0 2*pi],[0 0],'k-');
                xlabel('Time (s)');
                ylabel('Residuals');
...
output_values.amplitudes_err      = {amp_err, 'pixels'};
...
function [diff_sq nos] = SineSSD(coeff,X,Y, excess)
phase = coeff(1);
amplitude = coeff(2);
...
function [diff_sq nos] = SineSQ_SSD(coeff,X,Y,excess)
phase = coeff(1);
amplitude = coeff(2:3);
